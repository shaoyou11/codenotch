...
final class LocalizationTests: XCTestCase {
    ...
    func testCustomStatisticsTranslations() {
        XCTAssertEqual(L10n.t("Alert when \("Codex") crosses 80% and 100% of a limit.", locale: zhHans), "当 Codex 的额度越过 80% 和 100% 时提醒。")
        let value = "20.2"
    ...
    func testCustomNotificationAndPhoneTranslations() {
        XCTAssertEqual(L10n.t("When a limit resets", locale: zhHans), "额度重置时")
        XCTAssertEqual(L10n.t("Show notification for weekly limit", locale: zhHans), "提醒每周额度耗尽")
        XCTAssertEqual(L10n.t("Connect a Phone…", locale: zhHans), "连接手机…")
        XCTAssertEqual(L10n.t("\("Codex") Reset", locale: zhHans), "Codex 额度已重置")
        XCTAssertEqual(L10n.t("Ready on \("host"):\("1234")", locale: zhHans), "已就绪：host:1234")
        XCTAssertEqual(L10n.t("Today: \(2) requests · \("12") tokens in · \("8") out, across \(1) model(s) with history.", locale: zhHans), "今天：2 次请求 · 输入 12 个令牌 · 输出 8 个令牌，共 1 个模型有历史记录。")
        XCTAssertEqual(L10n.t("Tells you when a limit resets or runs out, speaks Russian, and reads Kimi.", locale: zhHans), "新增额度重置和耗尽提醒、俄语及 Kimi 支持。")
    }
    ...
    func testLimitNotificationCopyInSimplifiedChinese() {
        XCTAssertEqual(L10n.t("When a limit is reached", locale: zhHans), "额度耗尽时")
        XCTAssertEqual(L10n.t("Show notification for session limit", locale: zhHans), "提醒会话额度耗尽")
        XCTAssertEqual(L10n.t("Show notification for weekly limit", locale: zhHans), "提醒每周额度耗尽")
        ...
        XCTAssertEqual(L10n.t("Preview weekly limit alert", locale: zhHans), "预览每周额度提醒")
        XCTAssertEqual(
            ...
            "当服务的会话或每周额度耗尽时，在刘海旁显示提醒卡片。"
        )
        XCTAssertEqual(L10n.t("When a limit resets", locale: zhHans), "额度重置时")
        XCTAssertEqual(L10n.t("Show notification from notch", locale: zhHans), "从刘海显示提醒")
        XCTAssertEqual(L10n.t("Reset sound", locale: zhHans), "重置提示音")
        XCTAssertEqual(L10n.t("Preview notification", locale: zhHans), "预览提醒")
        XCTAssertEqual(
            L10n.t("Displays a notification card from the side of the notch when a provider's usage limit resets.", locale: zhHans),
            "当服务的使用额度重置时，在刘海旁显示提醒卡片。"
        )
    }
